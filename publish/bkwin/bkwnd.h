...
#ifdef BK_POS_SUPPORT_EXP
#include "bkarithmetic.h"
#endif
...
public:
    CBkWindow()
		...
		, m_bForceUpdateStyle(FALSE)
		, m_bAutoFixTextStringRect(TRUE)
		, m_pUserData(NULL)
		, m_bBorder(FALSE)
        , m_bSupportDpiAware(FALSE)

// 此宏定义是否默认开启：正确DrawBkgnd class skin
#ifdef __BK_CLASS_CORRECT_DRAW_BG__
		, m_bCorrectDrawBg(TRUE)
#else
		, m_bCorrectDrawBg(FALSE)
#endif
...
protected:
	...
	BOOL	m_bForceUpdateStyle;
	BOOL	m_bAutoFixTextStringRect;
	BOOL	m_bBorder;
    BOOL    m_bSupportDpiAware;
	UINT_PTR m_pUserData;
	BOOL m_bCorrectDrawBg;
public:
    ...
    void SetSupportDpiAware(BOOL bEnable)
    {
        m_bSupportDpiAware = bEnable;
    }

    long ScaleByDpi(long lSize)
    {
        return IsDpiAwareEnable() ? BKWinDpiAware::GetInstance().ScaleByDpi(lSize) : lSize;
    }

	long UnScaleByDpi(long lSize)
	{
		return IsDpiAwareEnable() ? BKWinDpiAware::GetInstance().UnScaleByDpi(lSize) : lSize;
	}

    BOOL IsDpiAwareEnable()
    {
        return BKWinDpiAware::GetInstance().IsDpiSwitchOn() && m_bSupportDpiAware;
    }
   ...
   virtual BOOL IsHeadAcceptLBtnDownMsg()
   {
    ...
    void SetParent(HBKWND hBkWndParent)
    {
        ...
        CBkWindow* pParentWnd = BkWnds::GetWindow(m_hBkWndParent);
        if (pParentWnd)
            SetSupportDpiAware(pParentWnd->m_bSupportDpiAware);
    }
...
//     }
	...
	HFONT GetFont(BOOL bCanUseDefault)
	{
        DWORD wFtText = 0;
        if (m_wFtText != 0)
            wFtText = m_wFtText;
        ...
        if (wFtText)
            m_ftText = BkFontPool::GetFont(wFtText, IsDpiAwareEnable());
        else if (bCanUseDefault)
            m_ftText = BkFontPool::GetFont(BKF_DEFAULTFONT, IsDpiAwareEnable());
	...
	void SetUserData(UINT_PTR pUserData)
	{
...
public:
    ...
    virtual BOOL NeedRedrawWhenStateChange()
    {
        ...
        if (!style.m_strSkinName.IsEmpty())
        {
            CBkSkinBase* pSkin = BkSkin::GetSkin(style.m_strSkinName);
            if (pSkin && !pSkin->IgnoreState())
                return TRUE;
        }

        return (CLR_INVALID != style.m_crHoverText) || (0 != style.m_wFtHover) || (CLR_INVALID != style.m_crBgHover) || 
			(CLR_INVALID != style.m_crPushDownText) || ( 0 != style.m_wFtPushDown);
    }
...
//     }
    ...
    void GetWndPos(WINDOWPOS& pos, CRect& rcParent)
    {
        BKDLG_POSITION dlgPos;
        GetDlgPosition(&dlgPos);

        if (dlgPos.Left.bCenter)
            pos.x = rcParent.left + rcParent.Width() / 2 + (dlgPos.Left.bMinus ? -1 : 1) *  dlgPos.Left.nPos;
        else if (dlgPos.Left.bMinus)
            pos.x = rcParent.left + rcParent.Width() - dlgPos.Left.nPos;
        else
            pos.x = rcParent.left + dlgPos.Left.nPos;

        if (dlgPos.Top.bCenter)
            pos.y = rcParent.top + rcParent.Height() / 2 + (dlgPos.Top.bMinus ? -1 : 1) *  dlgPos.Top.nPos;
        else if (dlgPos.Top.bMinus)
            pos.y = rcParent.top + rcParent.Height() - dlgPos.Top.nPos;
        else
            pos.y = rcParent.top + dlgPos.Top.nPos;

        pos.cx = rcParent.Width();
        pos.cy = rcParent.Height();
    }
...
public:
	...
	COLORREF GetDrawBackgroundColor() 
	{
    ...
    virtual void DrawBkgnd(CDCHandle& dc)
    {
		...
		if (m_bBorder)
		{
			dc.Rectangle(m_rcWindow.left,m_rcWindow.top,m_rcWindow.right,m_rcWindow.bottom);
		}
        ...
        if (theStyle.m_strSkinName.IsEmpty())
        {
			...
			if (CLR_INVALID != crBg)
			{
				if (theStyle.m_bCrBgGdiPlus)
				{
					...
					Gdiplus::SolidBrush		brush(Gdiplus::Color(theStyle.m_nAlphaBg, GetRValue(crBg), GetGValue(crBg), GetBValue(crBg)));
        ...
        else
        {
            ...
            if (pSkin)
            {
                //pSkin->Draw(dc, m_rcWindow, m_dwState, CBkSkinBase::SkinDrawMode_FullCtrlWindow_Stretch);
				if (m_bCorrectDrawBg) 
				{
				...
				{
					pSkin->Stretch(dc, m_rcWindow, m_dwState, IsDpiAwareEnable());
				}
            }
        }
    }
    ...
    void BeforePaint(CDCHandle &dc, BkDCPaint &BkDC)
    {
		...
		{
			bool bUsePushFont = false;
			bool bUsePushCrText = false;

			if (BkWndState_PushDown == (GetState() & BkWndState_PushDown))
			{
				if (theStyle.m_wFtPushDown) 
				{
					hftDraw = BkFontPool::GetFont(theStyle.m_wFtPushDown, IsDpiAwareEnable());
					bUsePushFont = true;
				}

				if (theStyle.m_crPushDownText != CLR_INVALID)
				{
					crDraw = theStyle.m_crPushDownText;
					bUsePushCrText = true;
				}
			}
			...
			{
				if (!bUsePushFont && theStyle.m_wFtHover)
					hftDraw = BkFontPool::GetFont(theStyle.m_wFtHover, IsDpiAwareEnable());

				if (!bUsePushCrText && theStyle.m_crHoverText != CLR_INVALID)
    ...
    void OnPaint(CDCHandle dc)
    {
		...
		if (0 == m_nTextAlign || m_bForceUpdateStyle) 
		...
		{
			int nAddSize = BkFontPool::GetFontSizeAdding(dc.GetCurrentFont(), IsDpiAwareEnable());
		...
		if (theStyle.m_nGdiplus)
		{
            ...
            if ((theStyle.m_dwTextModeEx & DT_SINGLELINE) == DT_SINGLELINE)
            {
                format.SetFormatFlags(Gdiplus::StringFormatFlagsLineLimit);
            }

            if ((theStyle.m_dwTextModeEx & DT_RIGHT) == DT_RIGHT)
            {
                format.SetAlignment(Gdiplus::StringAlignmentFar);
            }

            if ((theStyle.m_dwTextModeEx & DT_CENTER) == DT_CENTER)
            {
                format.SetAlignment(Gdiplus::StringAlignmentCenter);
            }

            if ((theStyle.m_dwTextModeEx & DT_VCENTER) == DT_VCENTER)
            {
                format.SetLineAlignment(Gdiplus::StringAlignmentCenter);
            }

			if ((theStyle.m_dwTextModeEx & DT_BOTTOM) == DT_BOTTOM)
			{
    ...
    LRESULT OnNcCalcSize(BOOL bCalcValidRects, LPARAM lParam)
    {
        ...
        else
        {
            ...
            if (BkWndState_Hover == (GetState() & BkWndState_Hover))
            {
                if (theStyle.m_wFtHover)
                    hftTest = BkFontPool::GetFont(theStyle.m_wFtHover, IsDpiAwareEnable());
                else
                {
                    CBkWindow* pWnd = this;
                    HBKWND hWndParent = NULL;
                    ...
                    {
                        pWnd = BkWnds::GetWindow(hWndParent);
                        if (pWnd && pWnd->GetStyle().m_wFtHover)
                        {
                            hftTest = BkFontPool::GetFont(pWnd->GetStyle().m_wFtHover, IsDpiAwareEnable());
                            break;
                        }
                    }
                }
            ...
            if (NULL == hftTest)
                hftTest = BkFontPool::GetFont(BKF_DEFAULTFONT, IsDpiAwareEnable());
			...
			if (0 != m_strInnerText.GetLength())
			{
				if (theStyle.m_bGdiplusCalcTxt)
				{
					...
					BkGdiplusFont font(dcTest, hftTest);
    ...
    HRESULT OnAttributePosChange(CStringA& strValue, BOOL bLoading)
    {
        if (!strValue.IsEmpty())
        {
            ...
            for (m_dlgpos.nCount = 0; m_dlgpos.nCount < 4; m_dlgpos.nCount ++)
            {
				...
				bool lw = 'w' == lpszValue[nPosPrev];
				if (lw) {
					nPosPrev++;
				}
				bool th = 'h' == lpszValue[nPosPrev];
				if (th) {
					nPosPrev++;
				}
                ...
                {
#ifdef BK_POS_SUPPORT_EXP
					BkArithmetic arithmetic;
					CStringA lpSrc = strValue.Mid(nPosPrev);
					lpSrc = arithmetic.InfixToSuffix(lpSrc.GetBuffer());
					m_dlgpos.Item[m_dlgpos.nCount].nPos = arithmetic.SuffixToArithmetic(lpSrc);//::StrToIntA(lpSrc);
#else
					m_dlgpos.Item[m_dlgpos.nCount].nPos = ::StrToIntA(strValue.Mid(nPosPrev));
#endif
					if (lw && m_dlgpos.nCount == 2) {
						m_dlgpos.Item[m_dlgpos.nCount].nPos += m_dlgpos.Item[0].nPos;
					}
					if (th && m_dlgpos.nCount == 3) {
						m_dlgpos.Item[m_dlgpos.nCount].nPos += m_dlgpos.Item[1].nPos;
					}
                    m_dlgpos.nCount++;
                    break;
                }
...
#ifdef BK_POS_SUPPORT_EXP
				BkArithmetic arithmetic;
				CStringA lpSrc = strValue.Mid(nPosPrev, nPos - nPosPrev);
				lpSrc = arithmetic.InfixToSuffix(lpSrc.GetBuffer());
                m_dlgpos.Item[m_dlgpos.nCount].nPos = arithmetic.SuffixToArithmetic(lpSrc);//::StrToIntA(lpSrc);
#else
            ...
            for (int i = 0; i < m_dlgpos.nCount; i++)
                m_dlgpos.Item[i].nPos = ScaleByDpi(m_dlgpos.Item[i].nPos);
	...
	HRESULT OnAttributeUserDataChange(CStringA& strValue, BOOL bLoading)
	{
		m_pUserData =  ::StrToIntA(strValue);
		return S_FALSE;
	}
...
//     };
    BKWIN_DECLARE_ATTRIBUTES_BEGIN()
        ...
        BKWIN_FONTSTRING_ATTRIBUTE("font", m_wFtText, FALSE)
		...
		BKWIN_CUSTOM_ATTRIBUTE("userdata", OnAttributeUserDataChange)//, 0)
		BKWIN_UINT_ATTRIBUTE("notify", m_nNotifyMsg, 0)
		BKWIN_INT_ATTRIBUTE("forceupdatestyle", m_bForceUpdateStyle, TRUE)
		BKWIN_INT_ATTRIBUTE("autofixtextrect", m_bAutoFixTextStringRect, TRUE)
		BKWIN_INT_ATTRIBUTE("border",m_bBorder,TRUE)
		BKWIN_INT_ATTRIBUTE("correctdrawbg", m_bCorrectDrawBg,TRUE)

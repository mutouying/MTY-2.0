...
#include "CDwmApi.h"
#include "bkrichtext2wnd.h"
#include "drag_message.h"
//#include "kbase/observer_list.h"
#include "bkownereventdlg.h"
#include "BkLineViewEx.h"
#include "BkSliderViewEx.h"
#include "BkDrawableView.h"
#include "BkSyncStatusDlgBtn.h"
#include "LinearGradientCtrl.h"

//UI监听布局改变
class IUILayoutObserver
{
public:
	virtual ~IUILayoutObserver()
	{}
public:
...
public:
	...
	CBkWindow* CreateBkControlByName( LPCSTR lpszName )
	{
		...
		pWnd = CBkRichText2Wnd::CheckAndNew(lpszName);
		if (pWnd) return pWnd;

		pWnd = CBkLineViewEx::CheckAndNew(lpszName);
		if (pWnd) return pWnd;

		pWnd = BkDrawableView::CheckAndNew(lpszName);
		if (pWnd) return pWnd;

		pWnd = CBkOwnerEventDlg::CheckAndNew(lpszName);
		if (pWnd) 
		{
			static_cast<CBkPanel*>(pWnd)->SetExternCreator(this);
			return pWnd;
		...
		pWnd = CBkSyncStatusDlgBtn::CheckAndNew(lpszName);
		if (pWnd) 
		{
			static_cast<CBkPanel*>(pWnd)->SetExternCreator(this);
			return pWnd;
		...
		pWnd = CBkSliderViewEx::CheckAndNew(lpszName);
		if (pWnd) 
		{
			static_cast<CBkPanel*>(pWnd)->SetExternCreator(this);
			return pWnd;
        ...
        pWnd = LinearGradientCtrl::CheckAndNew(lpszName);
        if (pWnd) return pWnd;
		if (m_pExtenCreator)
		{
...
protected:
	...
	//ObserverList<IUILayoutObserver>	m_UILayoutObserverList;
...
public:
	...
	HWND CreateEx(HWND hWndParent = ::GetActiveWindow(), UINT uCtrlID = 0, LPRECT rect = NULL)
	{
        ...
        HWND hWnd = NULL;
        BKEnableThreadAwarenessContextWarp threadDpiAwareWarp(m_bSupportDpiAware);
        hWnd = Create(hWndParent, NULL, GetDlgStyle(), GetDlgExStyle(), _U_MENUorID(uCtrlID));
...
//#endif
	...
	BOOL SetXml(LPCSTR lpszXml)
	{
...

		{
            ...
            pXmlRootElem->Attribute("dpiaware", &m_bSupportDpiAware);

            m_sizeDefault.cx = ScaleByDpi(m_sizeDefault.cx);
            m_sizeDefault.cy = ScaleByDpi(m_sizeDefault.cy);
        ...
        m_bkHeader.SetSupportDpiAware(m_bSupportDpiAware);
        ...
        m_bkFooter.SetSupportDpiAware(m_bSupportDpiAware);
        ...
        m_bkBody.SetSupportDpiAware(m_bSupportDpiAware);
	...
	virtual DWORD GetDlgStyle()
	{
		return m_dwDlgStyle;
	}

	virtual DWORD GetDlgExStyle()
	{
...
protected:
	...
	void _RepositionItems(BOOL bRedraw = TRUE)
	{
		...
		//FOR_EACH_OBSERVER(IUILayoutObserver,m_UILayoutObserverList,OnLayoutChange());
	}
	...
	void DoPaint(CDCHandle dc, UINT uFlags)
	{
		...
		if (uFlags == 0)
		{
			OnBkDialogViewEndDraw(m_imgMem);
	...
	virtual void OnBkDialogViewEndDraw(CBkBitmap& bmp)
	{
	...
	void OnLButtonDown(UINT nFlags, CPoint point)
	{
		...
		if (hBkWndHitTest)
		{
			...
			if (!(
				...
				pWndPushDown->IsClass("radio") ||
				pWndPushDown->IsClass("slider") ||
                pWndPushDown->IsClass("imgradio") ||
	...
	void OnLButtonDblClk(UINT nFlags, CPoint point)
	{
		...
		if (hBkWndHitTest)
		{
			...
			if (!(	pWndPushDown->IsClass("button") || 
				...
				pWndPushDown->IsClass("slider") ||
				pWndPushDown->IsClass("check") ||
				pWndPushDown->IsClass("listwnd")) && !pWndPushDown->IsHeadAcceptLBtnDownMsg())
			{
				if (m_bCanMaximize)
				{
					DWORD dwStyle = CWindow::GetStyle();
	...
	BOOL OnSetCursor(CWindow /*wnd*/, UINT nHitTest, UINT message)
	{
		if (m_hBkWndHover)
		{
			...
			if (pBkWndHover)
			{
				if (pBkWndHover->IsClass(CBkRealWnd::GetClassName()) || pBkWndHover->IsClass(CBkRealWnd2::GetClassName()))
					return FALSE;
	...
	void AddUILayoutObserver(IUILayoutObserver* observer)
	{
		m_UILayoutObserverList.AddObserver(observer);
	}

	void RemoveLayoutObserver(IUILayoutObserver* observer)
	{
		m_UILayoutObserverList.RemoveObserver(observer);
	}

protected:

	virtual void _ModalMessageLoop()
	{
		BOOL bRet;
		MSG msg;
...
// 		}
	...
	LRESULT OnMsgDragDrop(UINT uMsg, WPARAM wParam, LPARAM lParam )
	{
		...
		if (pDragHover != NULL)
		{
			pDragHover->BkSendMessage(DRAG_MSG::gMSG_Drop, wParam, lParam);
		}
	...
	LRESULT OnMsgDragOver(UINT uMsg, WPARAM wParam, LPARAM lParam )
	{
		...
		if (pDragHover != NULL)
		{
			pDragHover->BkSendMessage(DRAG_MSG::gMSG_DragOver, wParam, 0);
		}

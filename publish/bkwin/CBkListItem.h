...
public:
	...
	void SetItemHeightDirect(int nHeight) 
	{
		m_nItemHeigth = nHeight;
	}

	BOOL IsListItemViewVisible() const 
	{
		return m_bViewVisible;
	}

	void SetItemWidthDirect(int nWidth)
	{
		m_nItemWidth = nWidth;
	}
...
public:
	...
	virtual void DrawBkgnd(CDCHandle& dc)
	{
	 ...
	 HRESULT OnAttributeWidthChange(CStringA& strValue, BOOL bLoading)
	 {
		 ...
		 if (nWidth >=0 && nWidth != m_nItemWidth)
		 {		 
			 m_nItemWidth = nWidth;
			 m_bNeedReposeChild = TRUE;
    ...
    void ForceRepositionChildren()
    {
        _RepositionChilds();
    }

	void AddView(CBkWindow* pChild)
	{
		if (pChild != NULL)
		{
			pChild->SetParent(GetBkHWnd());
...
protected:
	...
	LRESULT SendNotifyMessage(CBkWindow* pWnd, UINT uListCode)
	{
		...
		if (strcmp(nms.childNotify.szItemClass, CBkCheckBox::GetClassName()) == 0)
		{
			nms.childNotify.checkbox.bItemCheckState = ((CBkCheckBox*) pWnd)->IsChecked();
			nms.childNotify.checkbox.dwItemCheckBoxUniqueId = ((CBkCheckBox*) pWnd)->GetUniqueId();
		}
		else
		{
			nms.childNotify.checkbox.bItemCheckState = FALSE;
			nms.childNotify.checkbox.dwItemCheckBoxUniqueId = 0;
		}
...
protected:
	...
	void OnWindowPosChanged(LPWINDOWPOS lpWndPos)
	{
		...
		if (!(lpWndPos->flags & SWP_NOREPOSITION))
        {
            m_bNeedReposeChild = TRUE;
        }

        if (m_bNeedReposeChild)
        {
            if (m_bViewVisible)
            {
...
protected:
	...
	BKWIN_DECLARE_ATTRIBUTES_BEGIN()
        ...
        BKWIN_INT_ATTRIBUTE("viewvisible", m_bViewVisible, FALSE)
